...
class ImageDeal():
    ...
    def imgsplice(self, filename):
        for i in range(1,3):#self.start,self.end):
            self.imglist.append(Image.open('%s.png'%i))
        result=Image.new('RGB',(self.imglist[0].size[0]*self.cols,self.imglist[0].size[1]*self.rows))
        ii=0
        for k in range(0,self.cols):#
            for j in range(0,self.rows):
                result.paste(self.imglist[ii],(k* self.imglist[0].size[0],j* self.imglist[0].size[1]))
                ii += 1
...
if __name__ == "__main__":
    img=ImageDeal(1,3,2,1)  #（起始数字,结束数字,行数,列数）
